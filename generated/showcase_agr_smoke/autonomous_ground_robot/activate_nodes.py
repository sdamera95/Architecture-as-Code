...
Usage (after launching nodes):
    ros2 run autonomous_ground_robot activate_nodes
...
def main(args=None):
    ...
    for node_name, namespace in LIFECYCLE_NODES:
        ...
        if call_transition(caller, node_name, namespace, Transition.TRANSITION_CONFIGURE):
            ...
            call_transition(caller, node_name, namespace, Transition.TRANSITION_ACTIVATE)
        time.sleep(0.5)

    caller.get_logger().info('All nodes activated.')
    caller.destroy_node()
    rclpy.shutdown()
